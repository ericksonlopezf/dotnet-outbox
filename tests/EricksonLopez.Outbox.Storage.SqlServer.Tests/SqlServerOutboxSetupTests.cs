...
public class SqlServerOutboxSetupTests
{
    ...
    [Fact]
    public void UseSqlServer_ReturnsOptionsInstanceForChaining()
    {
        var services = new ServiceCollection();
        OutboxOptions? returnedOptions = null;
        services.AddOutbox(options =>
        {
            returnedOptions = options.UseSqlServer(sp => Substitute.For<IDbConnection>());
            returnedOptions.Should().BeSameAs(options);
        });
        returnedOptions.Should().NotBeNull();
    }
}
