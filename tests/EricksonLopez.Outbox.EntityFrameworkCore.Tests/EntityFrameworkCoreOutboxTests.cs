...
public class EntityFrameworkCoreOutboxTests
{
    ...
    [Fact]
    public async Task FetchPendingAsync_WhenDbUpdateConcurrencyException_CatchesAndReturnsEmpty()
    {
        var interceptor = new TestSaveChangesInterceptor();
        var sp = CreateServiceProvider(Guid.NewGuid().ToString(), interceptor);
        ...
        using (var scope = sp.CreateScope())
        {
            ...
            db.Messages.Add(new OutboxMessageEntity
            {
                Id = Guid.NewGuid(),
                MessageType = "TestMsg",
                Payload = new byte[] { 1 },
        ...
        interceptor.ThrowConcurrencyException = true;
        var repo = sp.GetRequiredService<IOutboxRepository>();
        var fetched = await repo.FetchPendingAsync(10);
