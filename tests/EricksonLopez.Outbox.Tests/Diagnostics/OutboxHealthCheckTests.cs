...
public class OutboxHealthCheckTests
{
    ...
    private OutboxDispatcherBackgroundService CreateDispatcher(bool isRunning)
    {
        ...
        // White-box test rationale: simulates the running state without launching background threads/channel pipelines
        ReflectionTestHelper.SetFieldValue(dispatcher, "_isRunning", isRunning ? 1 : 0);
        return dispatcher;
    }
    ...
    public async Task CheckHealthAsync_ReturnsHealthy_WhenDispatcherRunning_AndUnderThreshold()
    {
        ...
        result.Data["storage_provider"].Should().Be(repo.GetType().Name);
    }
