    ...
    public class MetricsAndTelemetryTests
    {
        ...
        public async Task ProcessMessagesAsync_WhenHeadersContainTraceparent_PropagatesParentTraceToActivity()
        {
            ...
            using var listener = new System.Diagnostics.ActivityListener
            {
                ...
                ActivityStopped = a =>
                {
                    if (a.GetTagItem("messaging.destination.name")?.ToString() == "OrderCreated")
                    {
            ...
            var msg = new OutboxMessage(Guid.NewGuid(), "OrderCreated", ReadOnlyMemory<byte>.Empty, null, null, headersJson, DateTimeOffset.UtcNow, null, null, OutboxMessageStatus.Pending, 0, null);
            _publisher.PublishRawAsync(Arg.Any<OutboxMessage>(), Arg.Any<OutboxMessageMetadata>(), Arg.Any<DispatchContext>())
                .Returns(DispatchResult.Ok());
        ...
        public async Task ProcessMessagesAsync_WhenSkipExecutionTrue_DoesNotStartActivity()
        {
            ...
            using var listener = new System.Diagnostics.ActivityListener
            {
                ...
                ActivityStarted = a =>
                {
                    if (a.OperationName == "Outbox.Dispatch")
                    {
            ...
            var msg = new OutboxMessage(Guid.NewGuid(), "OrderCreated", new byte[10], null, null, ReadOnlyMemory<byte>.Empty, DateTimeOffset.UtcNow, null, null, OutboxMessageStatus.Pending, 0, null);
