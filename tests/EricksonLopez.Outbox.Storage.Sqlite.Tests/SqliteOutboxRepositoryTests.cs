...
public class SqliteOutboxRepositoryTests : IDisposable
{
    ...
    [Fact]
    public async Task InsertAsync_WhenOutboxMessagesTableName_InsertsSuccessfully()
    {
        var opt = new OutboxRuntimeOptions { TableName = "outbox_messages" };
        var sut = new SqliteOutboxRepository(() => new SqliteConnection(_connectionString), Microsoft.Extensions.Options.Options.Create(opt));
        var msg = CreateMessage();
    ...
    [Fact]
    public async Task FetchPendingAsync_WhenTableIsEmpty_ReturnsEmptyList()
    {
        var sut = CreateSut();
        var fetched = await sut.FetchPendingAsync(10);
    ...
    [Fact]
    public async Task PurgeDispatchedMessagesAsync_WhenBatchSizeIsZero_ReturnsZero()
    {
        var sut = CreateSut();
        var purged = await sut.PurgeDispatchedMessagesAsync(DateTimeOffset.UtcNow, batchSize: 0);
        purged.Should().Be(0);
    ...
    public async Task InsertBatchAsync_WhenRecordsEmpty_ReturnsImmediately()
    {
        ...
        await sut.InsertBatchAsync(ReadOnlyMemory<OutboxMessage>.Empty, new DbTransactionContext(tx));
        await tx.CommitAsync();

        var count = await connection.ExecuteScalarAsync<int>("SELECT COUNT(1) FROM messages");
        count.Should().Be(0);
    }
    ...
    {
        var sut = CreateSut();
        var initialCount = await sut.GetPendingCountAsync();
        initialCount.Should().Be(0);

        var msg1 = CreateMessage(state: 0);
        var msg2 = CreateMessage(state: 3);
        var msg3 = CreateMessage(state: 2); // Dispatched, not pending

        await using var connection = new SqliteConnection(_connectionString);
        await connection.OpenAsync();
        await using var tx = await connection.BeginTransactionAsync();
        await sut.InsertBatchAsync(new[] { msg1, msg2, msg3 }, new DbTransactionContext(tx));
        await tx.CommitAsync();

        await connection.ExecuteAsync("UPDATE messages SET state = 2 WHERE id = @Id", new { Id = msg3.Id.ToString() });

        var countAfter = await sut.GetPendingCountAsync();
