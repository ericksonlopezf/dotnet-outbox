...
public class OracleOutboxRepositoryTests : IAsyncLifetime
{
    ...
    [Fact]
    public void Constructor_Should_Allow_Custom_Schema()
    {
        var optionsMonitor = Substitute.For<IOptions<OutboxRuntimeOptions>>();
        optionsMonitor.Value.Returns(new OutboxRuntimeOptions { SchemaName = "app_schema", TableName = "outbox_messages" });
...
#pragma warning restore CS8765
    ...
    [Fact]
    public async Task InsertAsync_WithTransaction_Should_Rollback_On_Transaction_Rollback()
    {
        var sut = CreateSut();
        var msg = CreateMessage();
        ...
        var count = await newConn.ExecuteScalarAsync<int>("SELECT COUNT(1) FROM \"messages\" WHERE id = :Id", new { Id = msg.Id.ToByteArray() });
        count.Should().Be(0);
    }

    [Fact]
    public async Task InsertBatchAsync_WithTransaction_Should_Rollback_On_Transaction_Rollback()
    {
        var sut = CreateSut();
        var msg = CreateMessage();
    ...
    [Fact]
    public async Task InsertAsync_WithNullFields_PersistsNulls()
    {
        var sut = CreateSut();
        var msg = CreateMessage() with { DeliverAt = null, CorrelationId = null, CausationId = null };
        ...
        ((object?)row.DeliverAt).Should().BeNull();
    }

    [Fact]
    public async Task InsertAsync_WithNonNullDeliverAt_PersistsDeliverAt()
    {
        var sut = CreateSut();
        var deliverAt = DateTimeOffset.UtcNow.AddHours(2);
        var msg = CreateMessage() with { DeliverAt = deliverAt, CorrelationId = "corr-1", CausationId = "caus-1" };
        ...
        ((string?)row.CorrelationId).Should().Be("corr-1");
        ((string?)row.CausationId).Should().Be("caus-1");
        ((object?)row.DeliverAt).Should().NotBeNull();
    }

    [Fact]
    public async Task GetPendingCountAsync_EmptyTable_ReturnsZero()
    {
        await OracleTestDatabase.EnsureSchemaAsync(_fixture.Container.GetConnectionString(), "messages");
        var sut = CreateSut();
        var count = await sut.GetPendingCountAsync(CancellationToken.None);
    ...
    [Fact]
    public async Task FetchPendingAsync_WithNullFields_HydratesCorrectly()
    {
        var sut = CreateSut();
        var id = Guid.NewGuid();
        ...
        await connection.OpenAsync();
        await connection.ExecuteAsync(
            "INSERT INTO \"messages\" (id, type, correlation_id, causation_id, payload, headers_json, created_at, updated_at, deliver_at, processed_at, error, state) " +
            "VALUES (:Id, 'test.nulls', NULL, NULL, NULL, NULL, :CreatedAt, CURRENT_TIMESTAMP, NULL, NULL, NULL, 0)",
            new { Id = id.ToByteArray(), CreatedAt = createdAt });

        var fetched = await sut.FetchPendingAsync(10);
        var msg = fetched.FirstOrDefault(m => m.Id == id);
