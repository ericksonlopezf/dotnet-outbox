...
public static class MySqlTestDatabase
{
    public static async Task EnsureSchemaAsync(string connectionString, string tableName = "outbox_messages")
    {
        ...
        var schema = $@"
            ...
            TRUNCATE TABLE {tableName}_dead_letters;
            TRUNCATE TABLE {tableName}_idempotency;
            TRUNCATE TABLE `custom_schema`.`{tableName}_dead_letters`;
            TRUNCATE TABLE `custom_schema`.`{tableName}_idempotency`;
        ";
