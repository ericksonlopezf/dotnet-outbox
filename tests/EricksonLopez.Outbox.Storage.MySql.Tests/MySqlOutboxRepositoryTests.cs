...
public class MySqlOutboxRepositoryTests : IAsyncLifetime
{
    ...
    {
        await MySqlTestDatabase.EnsureSchemaAsync(_fixture.Container.GetConnectionString() + ";AllowLoadLocalInfile=true", _options.TableName);
    }

    public Task DisposeAsync() => Task.CompletedTask;

    private MySqlOutboxRepository CreateSut(OutboxRuntimeOptions? customOptions = null)
    {
        return new MySqlOutboxRepository(
            () => new MySqlConnection(_fixture.Container.GetConnectionString() + ";AllowLoadLocalInfile=true"),
    ...
    {
        Action act = () => { _ = new MySqlOutboxRepository(null!); };
    ...
    public void Constructor_InvalidSchemaName_ThrowsArgumentException(string schema)
    {
        ...
        Action act = () => { _ = new MySqlOutboxRepository(() => new MySqlConnection(), Options.Create(opt)); };
    ...
    {
        var sut = new MySqlOutboxRepository(() => new MySqlConnection(), null);
    ...
    public async Task FetchPendingAsync_Should_Map_NonNull_Fields_Correctly()
    {
        ...
        System.Text.Encoding.UTF8.GetString(f.Payload.Span).Should().Be("{\"mysql_custom\":\"payload_value\"}");
        System.Text.Encoding.UTF8.GetString(f.Headers.Span).Should().Be("{\"mysql_custom\":\"header_value\"}");
    }
