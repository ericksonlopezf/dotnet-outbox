...
public sealed class InboxCleanupBackgroundServiceTests
{
    ...
    public async Task ExecuteAsync_WhenEnabled_PurgesEntriesPeriodicallyBasedOnRetention_AndLogsLifecycle()
    {
        ...
        var purge1Tcs = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        var purge2Tcs = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        var purgeCalls = 0;
        store.PurgeExpiredEntriesAsync(Arg.Any<DateTimeOffset>(), Arg.Any<CancellationToken>())
            .Returns(_ =>
            {
                var c = Interlocked.Increment(ref purgeCalls);
                if (c == 1) purge1Tcs.TrySetResult(true);
                if (c == 2) purge2Tcs.TrySetResult(true);
        ...
        logger.When(l => l.Log(
            ...
            Arg.Any<Exception?>(),
            Arg.Any<Func<object, Exception?, string>>()))
            .Do(_ => startedTcs.TrySetResult(true));

        var service = new InboxCleanupBackgroundService(store, options, logger, fakeTime);
        using var cts = new CancellationTokenSource();

        await service.StartAsync(cts.Token);
        await startedTcs.Task.WaitAsync(TimeSpan.FromSeconds(10));
        await Task.Delay(50);
        ...
        fakeTime.Advance(TimeSpan.FromMinutes(5));
        await purge1Tcs.Task.WaitAsync(TimeSpan.FromSeconds(10));
        ...
        await purge2Tcs.Task.WaitAsync(TimeSpan.FromSeconds(10));
    ...
    public async Task ExecuteAsync_WhenPurgeThrows_LogsErrorAndContinuesLoop()
    {
        ...
        logger.When(l => l.Log(
            ...
            dbEx,
            Arg.Any<Func<object, Exception?, string>>()))
            .Do(_ => errorLoggedTcs.TrySetResult(true));
        ...
        await errorLoggedTcs.Task.WaitAsync(TimeSpan.FromSeconds(10));
    ...
    public async Task ExecuteAsync_WhenPurgeThrowsUncanceledOperationCanceledException_LogsErrorAndContinues()
    {
        ...
        var store = Substitute.For<IInboxStore>();
        var opExLoggedTcs = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        store.PurgeExpiredEntriesAsync(default, default)
        ...
        logger.When(l => l.Log(
            ...
            opEx,
            Arg.Any<Func<object, Exception?, string>>()))
            .Do(_ => opExLoggedTcs.TrySetResult(true));
        ...
        fakeTime.Advance(TimeSpan.FromMinutes(6));
        await opExLoggedTcs.Task.WaitAsync(TimeSpan.FromSeconds(10));
