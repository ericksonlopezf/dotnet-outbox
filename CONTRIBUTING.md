...
Before opening a Pull Request, ensure you haven't broken the mutation score. Install the Stryker CLI and run it with the provided configuration:
...
To run mutation testing against only the core package:

```bash
dotnet stryker -f stryker-config-unit.json
```

**Thresholds** (from `stryker-config.json`):
- **Target (High):** 100%
- **Warning (Low):** 98%
- **Build Break:** 95% — CI will reject any PR that drops below this threshold.
...
Once you submit your PR, the GitHub Actions pipeline will validate your code against SonarCloud, Codecov, and NativeAOT compatibility. A maintainer will review your code shortly after.
