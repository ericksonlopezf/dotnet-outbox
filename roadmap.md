...
## ⛔ Formally Discarded / Out of Scope (Non-Goals)

In accordance with architectural ADRs, the following capabilities are **explicitly excluded** from the core Outbox scope:

1. **Saga / Process Manager Orchestration (ADR-019)**: Sagas are multi-step state machines. Outbox is an atomic publish primitive. Sagas belong to dedicated workflow engines (e.g. MassTransit Sagas, Wolverine).
2. **In-Process Event Bus / Mediator (ADR-016)**: Outbox does not dispatch events to in-process handlers. In-process dispatch belongs to `EricksonLopez.Mediator`.
3. **Domain Event Ownership (ADR-018)**: Outbox never owns domain events; aggregates own domain events, which are mapped to integration messages at the application boundary.
4. **Exactly-Once Delivery (ADR-017)**: Exactly-once delivery across heterogeneous DB + Broker networks is impossible without distributed 2PC/XA. The outbox guarantees **At-Least-Once** delivery.
5. **Generic Job Scheduling (ADR-025)**: `DeliverAt` provides delivery delay for messages; it is not a recurring cron/job scheduler (use Quartz or Hangfire).
6. **Built-in CDC / Transaction Log Tailing in Core**: WAL/CDC tailing is infrastructure-level transport, not part of the transactional outbox core library.
7. **Admin UI Dashboard in Core**: Outbox core remains minimal, headless, and zero-allocation. Any dashboard tooling will be developed as external tooling.
