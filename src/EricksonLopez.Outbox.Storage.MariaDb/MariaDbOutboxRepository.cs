...
/// <summary>
/// Provides a MariaDB-specific implementation of <see cref="IOutboxRepository"/>.
/// </summary>
/// <remarks>
/// Although MariaDB shares the <c>MySqlConnector</c> driver with MySQL, this implementation
/// uses a plain multi-row <c>INSERT IGNORE</c> for batch inserts instead of
/// <c>MySqlBulkCopy</c>, which is not supported by MariaDB.
/// </remarks>
public sealed class MariaDbOutboxRepository : IOutboxRepository
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MariaDbOutboxRepository"/> class.
    /// </summary>
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// MariaDB does not support <c>MySqlBulkCopy</c> (which relies on the MySQL-specific
    /// <c>LOCAL INFILE</c> protocol). This implementation uses a standard multi-row
    /// <c>INSERT IGNORE INTO … VALUES (…),(…)</c> statement instead.
    /// </remarks>
    public async ValueTask InsertBatchAsync(ReadOnlyMemory<OutboxMessage> records, IOutboxTransactionContext transaction, CancellationToken cancellationToken = default)
    {
        ...
        // Build a parameterized multi-row INSERT IGNORE
        var sb = new StringBuilder();
        sb.Append("INSERT IGNORE INTO ").Append(_fullTableName)
          .Append(" (id, type, payload, correlation_id, causation_id, headers_json, state, created_at, updated_at, deliver_at, retry_count) VALUES ");
        ...
        {
            if (i > 0) sb.Append(',');
            sb.Append(CultureInfo.InvariantCulture, $"(@Id{i},@Type{i},@Payload{i},@CorrId{i},@CausId{i},@Headers{i},0,@CreatedAt{i},@CreatedAt{i},@DeliverAt{i},0)");

            var r = records.Span[i];
            cmd.Parameters.Add(new MySqlParameter($"@Id{i}", r.Id));
            cmd.Parameters.Add(new MySqlParameter($"@Type{i}", r.MessageType));
            cmd.Parameters.Add(new MySqlParameter($"@Payload{i}", r.Payload.ToArray()));
            cmd.Parameters.Add(new MySqlParameter($"@CorrId{i}", r.CorrelationId ?? (object)DBNull.Value));
            cmd.Parameters.Add(new MySqlParameter($"@CausId{i}", r.CausationId ?? (object)DBNull.Value));
            cmd.Parameters.Add(new MySqlParameter($"@Headers{i}", r.Headers.ToArray()));
            cmd.Parameters.Add(new MySqlParameter($"@CreatedAt{i}", r.CreatedAt.UtcDateTime));
            cmd.Parameters.Add(new MySqlParameter($"@DeliverAt{i}", r.DeliverAt.HasValue ? r.DeliverAt.Value.UtcDateTime : (object)DBNull.Value));
        }

        // Stryker disable once Statement : Optional trailing SQL statement delimiter per ADR-013
        sb.Append(';');
