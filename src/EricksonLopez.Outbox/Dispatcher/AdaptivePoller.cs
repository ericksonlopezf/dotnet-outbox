...
internal sealed class AdaptivePoller : IPollerWakeup, IDisposable
{
    ...
    // Stryker disable Block : Dispose body cannot be meaningfully removed per ADR-013
    public void Dispose()
    {
        // Stryker disable once Statement : Disposable cleanup per ADR-013
    ...
    public async Task StartPollingAsync(CancellationToken cancellationToken)
    {
        ...
        // Stryker restore Boolean,Conditional,Equality
        {
            ...
            if (fetchedCount >= _options.BatchSize && _options.UseAdaptivePolling)
            {
                ...
                // Stryker disable once Conditional,Equality,Arithmetic : MaxBatchesPerSecond rate-limiter fallback per ADR-013
            ...
            try
            {
                ...
                // Stryker disable once Statement : CTS coordination cancel per ADR-013
                cts.Cancel(); // Cancel the other task
                // Stryker disable once Statement : Awaiting completed task for structured exception propagation per ADR-013
                await completedTask;
            }
