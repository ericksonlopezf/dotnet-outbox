...
internal sealed class OutboxChannel
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            while (await _channel.Reader.WaitToReadAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                // Stryker disable once Statement : Cache reset for memory correctness per ADR-013
                headersCache.Reset();
    ...
    private async ValueTask ExecuteDbWithRetryAsync(Func<CancellationToken, ValueTask> operation, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            // Stryker disable Boolean,Conditional,Equality,Relational : Retry limit check per ADR-013
            catch (Exception ex) when (ex is not OperationCanceledException && attempt < maxAttempts)
            // Stryker restore Boolean,Conditional,Equality,Relational
            {
                attempt++;
        ...
        out DispatchResult result)
    {
        ...
        try
        {
            ...
            if (!message.Headers.IsEmpty)
            {
                ...
                // Stryker disable String : Header key names are observable API contract per ADR-013
        ...
        if (_includeMessageTypeTag)
        {
            _metrics.DispatchFailures.Add(1,
                new System.Diagnostics.TagList
                {
                    ...
                    // Stryker disable once String : Metrics tag key is observable API contract per ADR-013
    ...
    private void RecordProcessMetrics(OutboxMessage message, bool skipExecution, long swTicks)
    {
        ...
        // Stryker disable once Statement : Telemetry recording per ADR-013
    ...
    {
        // Stryker disable once String,Statement : Log message formatting per ADR-013
