...
public sealed class OutboxMessageAnalyzer : DiagnosticAnalyzer
{
    ...
    private static bool TryGetTimeSpanSeconds(IOperation operation, out double seconds)
    {
        ...
        if (operation is IPropertyReferenceOperation prop &&
            prop.Property.Name == "Zero" &&
            prop.Property.ContainingType?.Name == "TimeSpan")
        {
            seconds = 0;
            return true;
        }
        ...
        if (operation.Syntax is InvocationExpressionSyntax invSyntax &&
            invSyntax.Expression is MemberAccessExpressionSyntax memberAccess &&
            memberAccess.Expression is IdentifierNameSyntax typeId &&
            typeId.Identifier.Text == "TimeSpan")
        {
            ...
            if (firstArg?.Expression is LiteralExpressionSyntax lit &&
                lit.Token.Value is object litVal)
            {
                double num = Convert.ToDouble(litVal, System.Globalization.CultureInfo.InvariantCulture);
                switch (methodName)
                {
                    case "FromSeconds":
                        seconds = num;
        ...
        else if (operation.Syntax is MemberAccessExpressionSyntax zeroAccess &&
                 zeroAccess.Expression is IdentifierNameSyntax zeroTypeId &&
                 zeroTypeId.Identifier.Text == "TimeSpan" &&
                 zeroAccess.Name.Identifier.Text == "Zero")
        {
            seconds = 0;
            return true;
        }
