...
## 🏆 Legend
- **🚀 EricksonLopezOutbox**: The reference implementation under test (Our Library).
- **🔧 CAP**: `DotNetCore.CAP` competitor baseline.
- **📦 NServiceBus**: `NServiceBus` competitor baseline.

## 📊 Quick Visual Comparison
| Benchmark | EricksonLopez | CAP | NServiceBus | Winner |
|---|---|---|---|:---:|
| **StoreAsync (Latency)** | 256 ns | 856 ns | 25,424 ns | 🏆 **EricksonLopez** |
| **StoreAsync (Allocated)** | 448 B | 1664 B | 5457 B | 🏆 **EricksonLopez** |
| **StoreAsync (Ops/Sec)** | ~3.9M | ~1.1M | ~39K | 🏆 **EricksonLopez** |
| **Serialize (Latency)** | 65.8 ns | — | — | 🏆 **EricksonLopez** |
| **Serialize (Allocated)** | 32 B | — | — | 🏆 **EricksonLopez** |

---

## 🔬 Benchmark Context
To guarantee a fair and deterministic comparison, the environment for the main tests is configured as follows:
- **Payload**: ~512 bytes JSON (modifiable in parameterized scenarios).
- **Serializer**: `System.Text.Json` with Source Generators (NativeAOT ready).
- **Database / Storage**: `InMemory` (To isolate framework overhead from network I/O).
- **Transaction**: Fake wrapper (Dummy Transaction) without real database persistence.
- **Hardware**: Measured to eliminate network noise; purely represents computational cost (CPU) and Garbage Collector pressure (RAM).

> **What exactly does `StoreAsync` represent in these benchmarks?**
> In these tests, `StoreAsync` strictly measures:
> 1. Message type resolution.
> 2. Metadata construction (CorrelationId, Dates, Type).
> 3. Payload serialization to JSON.
> 4. Routing through registered middlewares (Pipeline).
> 5. Writing the resulting object to the underlying in-memory collection.
> *(Does not include network latency to PostgreSQL, as that depends on the user's infrastructure).*

---

## 1. Competitor Benchmarks (StoreAsync)
```text
BenchmarkDotNet v0.13.12, Windows 11 (10.0.26200.8875)
.NET SDK 10.0.302
  [Host]     : .NET 10.0.10, X64 RyuJIT AVX-512F
```
| Method                               | Mean        | Min         | Max         | P50         | P95         | Ops/Sec   | Gen0   | Allocated |
|------------------------------------- |------------:|------------:|------------:|------------:|------------:|----------:|-------:|----------:|
| 🚀 **EricksonLopezOutbox_StoreAsync** |    256.3 ns |    254.3 ns |    258.6 ns |    256.0 ns |    258.0 ns | 3,901,677 | 0.0086 |     448 B |
| 🔧 CAP_StoreAsync                      |    855.7 ns |    841.0 ns |    867.8 ns |    855.0 ns |    866.0 ns | 1,168,633 | 0.0305 |    1664 B |
| 📦 NServiceBus_StoreAsync              | 25,423.8 ns | 25,021.9 ns | 25,754.8 ns | 25,400.0 ns | 25,700.0 ns |    39,333 | 0.0610 |    5457 B |

*(Note: The relative variability -Coefficient of Variation- is less than 1% in our library, indicating extremely high stability and the absence of long GC pauses).*

---

## 2. Scalability: Batch Store Benchmarks
This benchmark demonstrates the framework's behavior under massive load. Many frameworks degrade severely when the number of messages in a transaction grows.

```text
Job-BATCH : .NET 10.0.10, X64 RyuJIT AVX-512F
```
| Method                                | BatchSize | Mean           | P50            | P95            | Ops/Sec     | Gen0    | Gen1    | Gen2 | LOH | Allocated |
|-------------------------------------- |----------:|---------------:|---------------:|---------------:|------------:|--------:|--------:|-----:|----:|----------:|
| 🚀 **EricksonLopezOutbox_Store_Batch** |         1 |       280.2 ns |       280.5 ns |       282.0 ns | 3,569,167.3 |  0.0067 |       - |    - |   - |     344 B |
| 🚀 **EricksonLopezOutbox_Store_Batch** |       100 |    21,354.8 ns |    21,385.6 ns |    21,558.6 ns |    46,827.9 |  0.6714 |  0.0305 |    - |   - |   34640 B |
| 🚀 **EricksonLopezOutbox_Store_Batch** |      1000 |   215,339.5 ns |   215,551.5 ns |   217,281.6 ns |     4,643.8 |  6.8359 |  5.8594 |    - |   - |  350955 B |
| 🚀 **EricksonLopezOutbox_Store_Batch** |     10000 | 2,169,871.1 ns | 2,170,711.7 ns | 2,217,137.5 ns |       460.9 | 66.4063 | 58.5938 |    - |   - | 3518509 B |

---

## 3. Concurrency Benchmarks
Evaluates how the library scales when multiple threads attempt to publish to the Outbox simultaneously, checking for locks or shared resource contention.

```text
Job-PARALLEL : .NET 10.0.10, X64 RyuJIT AVX-512F
```
| Method                                   | Threads | Mean       | P95        | Ops/Sec     | Gen0   | Allocated |
|----------------------------------------- |--------:|-----------:|-----------:|------------:|-------:|----------:|
| 🚀 **EricksonLopezOutbox_Store_Parallel** |       1 |   846.7 ns |   864.4 ns | 1,181,111.4 | 0.0143 |     728 B |
| 🚀 **EricksonLopezOutbox_Store_Parallel** |       4 | 1,545.6 ns | 1,579.6 ns |   646,998.9 | 0.0515 |    2600 B |
| 🚀 **EricksonLopezOutbox_Store_Parallel** |      16 | 4,474.8 ns | 5,529.0 ns |   223,472.4 | 0.1907 |    9800 B |
| 🚀 **EricksonLopezOutbox_Store_Parallel** |      64 | 9,699.6 ns | 9,879.6 ns |   103,097.0 | 0.7782 |   38601 B |

---

## 4. End-to-End Pipeline
Measures the complete lifecycle of the Outbox: `Publish() -> Serialize -> Middleware -> InMemoryStore -> Commit -> Poller -> Dispatch -> DeadLetter (if fails)`. This is the definitive infrastructure impact benchmark.

```text
Job-E2E : .NET 10.0.10, X64 RyuJIT AVX-512F
```
| Method                                    | Mean     | P50       | P95      | Ops/Sec  | Gen0 | Allocated |
|------------------------------------------ |---------:|----------:|---------:|---------:|-----:|----------:|
| 🚀 **EricksonLopezOutbox_Synthetic_E2E**   | 16.70 μs | 20.450 μs | 23.36 μs | 59,894.6 |    - |    8800 B |

---

## 5. Serialization by Payload Size
Compares how the serialization pipeline behaves (allocations and latency) when varying the message size from small (512 B) to large (100 KB).

```text
IterationCount=100  WarmupCount=20  
```
| Method                                            | Payload | Mean        | P95         | Ratio | Gen0    | Gen1   | LOH | Allocated |
|-------------------------------------------------- |--------:|------------:|------------:|------:|--------:|-------:|----:|----------:|
| 🚀 **EricksonLopezOutbox_Serialize_Allocating**    |   512 B |    89.43 ns |    91.61 ns |  1.00 |  0.0117 |      - |   - |     592 B |
| 🚀 **EricksonLopezOutbox_Serialize_BufferWriter**  |   512 B |    65.84 ns |    66.33 ns |  0.74 |  0.0006 |      - |   - |      32 B |
| 🚀 **EricksonLopezOutbox_Serialize_Allocating**    |   10 KB |   593.30 ns |   636.67 ns |  1.00 |  0.2050 | 0.0076 |   - |   10320 B |
| 🚀 **EricksonLopezOutbox_Serialize_BufferWriter**  |   10 KB |   336.76 ns |   338.50 ns |  0.58 |  0.0005 |      - |   - |      32 B |
| 🚀 **EricksonLopezOutbox_Serialize_Allocating**    |  100 KB | 7,766.59 ns | 7,800.02 ns |  1.00 | 17.7460 | 17.7460|   - |  102573 B |
| 🚀 **EricksonLopezOutbox_Serialize_BufferWriter**  |  100 KB | 3,379.60 ns | 3,392.31 ns |  0.44 |       - |      - |   - |      32 B |

---

## 6. Message Construction Benchmarks
Measures the purely in-memory overhead of assembling the wrapper and metadata before serializing.

| Method                                              | Mean      | Min       | Max       | Ops/Sec       | Gen0   | Allocated |
|---------------------------------------------------- |----------:|----------:|----------:|--------------:|-------:|----------:|
| 🚀 **EricksonLopezOutbox_CreateOutboxMessage**      | 78.069 ns | 76.537 ns | 79.067 ns |  12,809,150.2 | 0.0041 |     208 B |
| 🚀 **EricksonLopezOutbox_CreateMessageMetadata**    |  2.390 ns |  2.343 ns |  2.498 ns | 418,370,261.0 | 0.0011 |      56 B |
